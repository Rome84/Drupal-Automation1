...
printf ${green}"Starting Docker container: Rome84/docker-$distro-ansible."${neutral}"\n"
docker pull Rome84/docker-$distro-ansible:latest
docker run --detach --volume="$PWD":/etc/ansible/roles/role_under_test:rw --name $container_id $opts Rome84/docker-$distro-ansible:latest $init
